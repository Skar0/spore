...
def pg2bdd_direct_encoding(pg_path, manager):
    ...
    with open(pg_path, "r") as pg_file:
        ...
        for line in pg_file:
            ...
            vertex_dict = tools.int2dict(index, vars)
            vertex_bdd = manager.cube(vertex_dict)  # create a BDD node for this valuation
        ...
        for line in pg_file:
            ...
            for succ in infos[3].split(","):
                successor = int(succ)
                index_bdd = manager.cube(tools.int2dict(index, vars))
                succ_bdd = manager.let(mapping_bis, manager.cube(tools.int2dict(successor, vars)))
